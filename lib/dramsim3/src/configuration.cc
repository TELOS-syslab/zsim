...
Config::Config(std::string config_file, std::string out_dir, std::string prefix)
    : output_dir(out_dir), reader_(new INIReader(config_file)), output_prefix(prefix) {
...
void Config::InitOtherParams() {
    ...
    if (output_prefix.empty()) {
        output_prefix = reader.Get("other", "output_prefix", "dramsim3");
    }
    json_stats_name = output_dir + output_prefix + ".json";
    json_epoch_name = output_dir + output_prefix + "-epoch.json";
    txt_stats_name = output_dir + output_prefix + ".txt";
    return;
}
...
void Config::InitTimingParams() {
    ...
    tCCD_L_WR = GetInteger("timing", "tCCD_L_WR", 0);
