...
struct DS3Request {
    DS3Request(uint64_t addr, uint64_t cycle) : addr(addr), channel(0), rank(0), bank(0), row(0), added_cycle(cycle) {}
    uint64_t addr;
    uint64_t channel;
    uint64_t rank;
    uint64_t bank;
...
class DRAMSim3Memory : public MemObject {  // one DRAMSim3 controller
   private:
    g_string name;
    const uint32_t controllerSysLatency;
    uint32_t minLatency;
