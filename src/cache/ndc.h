...
class NDCScheme : public CacheScheme {
   private:
    ...
    uint64_t _index_mask, _tag_mask;
    uint32_t _num_tag_bits, _num_index_bits, _num_addr_bits, _num_shift_bits;
   ...
   public:
    NDCScheme(Config& config, MemoryController* mc) : CacheScheme(config, mc) {
        ...
        _num_tag_bits = ceil(log2(_num_ways));
        _num_index_bits = ceil(log2(_num_sets));
        _num_addr_bits = _num_tag_bits + _num_index_bits;
        assert(_num_addr_bits <= MAX_ADDR_BITS);
        assert(_num_tag_bits <= _co_mask + 1);
        ...
        if (index_mask_upper == 0 && index_mask_lower == 0) {
            // Create default index mask with bits set from bit position _num_tag_bits upward
            _index_mask = ((1ULL << _num_index_bits) - 1) << _num_tag_bits;
        } else {
        ...
        _tag_mask = ~_index_mask & ((1ULL << _num_addr_bits) - 1);
        ...
        for (int i = _num_addr_bits - 1; i >= 0; i--) {
            index_mask_str += ((_index_mask >> i) & 1) ? '1' : '0';
            tag_mask_str += ((_tag_mask >> i) & 1) ? '1' : '0';
        ...
        info("index_mask = 0x%lx (%s); tag_mask = 0x%lx (%s)\n", _index_mask, index_mask_str.c_str(), _tag_mask, tag_mask_str.c_str());
    }
    ...
    inline Address phyAddr2cacheAddr(Address phy_addr) {
        ...
        int first_tag_bit = 0;
        while (first_tag_bit < 64 && !(_tag_mask & (1ULL << first_tag_bit))) {
            first_tag_bit++;
        }

        // Extract tag bits using direct bit manipulation
        for (int i = first_tag_bit; i < _num_addr_bits && bit_pos < _num_tag_bits; i++) {
            if (_tag_mask & (1ULL << i)) {
        ...
        for (int i = first_index_bit; i < _num_addr_bits && bit_pos < _num_index_bits; i++) {
        ...
        bit_pos = 0;
        for (int i = 0; i < _num_addr_bits && bit_pos < _num_index_bits; i++) {
            // Skip the tag positions
            if (i >= _co_pos && i < (_co_pos + _num_tag_bits)) {
                continue;
            }
        ...
        cache_addr = (cache_addr << _num_shift_bits) | (phy_addr & ((1ULL << _num_shift_bits) - 1));
        return cache_addr;
    }
    ...
    inline uint64_t getTag(Address cache_addr) {
        uint64_t hex_addr = cache_addr >> _num_shift_bits;
        return (hex_addr >> _co_pos) & ((1ULL << _num_tag_bits) - 1);
    }
