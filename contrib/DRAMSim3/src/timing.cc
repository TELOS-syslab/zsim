...
Timing::Timing(const Config& config)
    ...
    int write_to_write_l_wr = std::max(config.burst_cycle, config.tCCD_L_WR);
    ...
    same_bank[static_cast<int>(CommandType::WRITE)] =
        std::vector<std::pair<CommandType, int> >{
            ...
            {CommandType::WRITE, std::max(write_to_write_l, write_to_write_l_wr)},
            {CommandType::READ_PRECHARGE, write_to_read_l},
            {CommandType::WRITE_PRECHARGE, std::max(write_to_write_l, write_to_write_l_wr)},
